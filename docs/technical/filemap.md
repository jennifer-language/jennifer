...
| `scripts/gen-module-docs.j` | Jennifer program that renders the per-module API reference (`docs/api/`) + the `docs/SUMMARY.md` "Module API Reference" region from each module's doc comments, via the `docblock` module. Committed output; CI (`test.yml`) regenerates and fails on drift. |
| `docs/api/` | GENERATED by `scripts/gen-module-docs.j` (committed, do not hand-edit). One API page per shipped module plus `index.md`. |
