...
| `scripts/gen-module-docs.j` | Jennifer program that renders the per-module API reference (`docs/api/`) + the `docs/SUMMARY.md` "Module API Reference" region from each module's doc comments, via the `docblock` module. Run at publish time by `docs.yml` before `mdbook build` (not committed); `test.yml` runs it as a smoke-check. |
