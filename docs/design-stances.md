...
| 2   | **Explicit over implicit.**                              | Sigils mark use-site references (`$x`), `def` carries the type, libraries are imported per topic (`use io;`; nothing auto-loads except `core`), conditions must be `bool` (no truthiness), conversions are spelled out (`convert.toInt(v)`, `convert.toFloat(v)`). Nothing important hides. |
...
| 7   | **Topic-based, opt-in libraries.**                       | The standard library is split by topic, never bundled. Every library except the small auto-loaded `core` is enabled explicitly with `use NAME;`.                                                                                                                                            |
