...
def f as feed.Feed init feed.feedUpdated(
    feed.feed("Jennifer Release Notes", "https://jennifer-language.dev"),
    $built);

def one as feed.Entry init feed.entrySummary(
    feed.entryPublished(
        feed.entryId(
            feed.entry("0.19 ships the screen module", "https://jennifer-language.dev/notes/0-19"),
            "note-0-19"),
...
def two as feed.Entry init feed.entrySummary(
    feed.entryPublished(
        feed.entryId(
            feed.entry("0.18 adds the feed module", "https://jennifer-language.dev/notes/0-18"),
            "note-0-18"),
...
io.printf(
    ...
    $fromRss.title,
    len($fromRss.entries),
    time.iso($fromRss.updated));
io.printf(
    "parsed Atom: \"%s\", %d entries, updated %s\n",
    $fromAtom.title,
    len($fromAtom.entries),
    time.iso($fromAtom.updated));
