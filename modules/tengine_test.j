...
func testIntAndFloatOutput() {
    testing.assertEqual(
        render(oneSet('{{ .n }}/{{ .f }}'), "main", json.decode('{"n":42,"f":1.5}')),
        "42/1.5");
}
...
func testRangeOfObjects() {
    testing.assertEqual(
        render(
            oneSet('{{ range .ps }}<{{ .t }}>{{ end }}'),
            "main",
            json.decode('{"ps":[{"t":"one"},{"t":"two"}]}')),
        "<one><two>");
}
...
func testWith() {
    testing.assertEqual(
        render(
            oneSet('{{ with .obj }}{{ .name }}{{ end }}'),
            "main",
            json.decode('{"obj":{"name":"Doc"}}')),
        "Doc");
}

func testPipeUpper() {
    testing.assertEqual(
        render(oneSet('{{ .a | upper }}'), "main", json.decode('{"a":"hi"}')),
        "HI");
}

func testPipeTitle() {
    testing.assertEqual(
        render(oneSet('{{ .t | title }}'), "main", json.decode('{"t":"time travel"}')),
        "Time Travel");
}

func testPipeChain() {
    testing.assertEqual(
        render(oneSet('{{ .t | upper | trim }}'), "main", json.decode('{"t":"  hi  "}')),
        "HI");
}

func testPipeHtmlEscape() {
    testing.assertEqual(
        render(oneSet('{{ .h | html }}'), "main", json.decode('{"h":"<b>&"}')),
        "&lt;b&gt;&amp;");
}

func testPipeUrlize() {
    testing.assertEqual(
        render(oneSet('{{ .t | urlize }}'), "main", json.decode('{"t":"Time Travel"}')),
        "time-travel");
}
...
func testNestedIfInRange() {
    testing.assertEqual(
        render(
            oneSet('{{ range .xs }}{{ if .on }}{{ .v }}{{ end }}{{ end }}'),
            "main",
            json.decode('{"xs":[{"on":true,"v":"a"},{"on":false,"v":"b"}]}')),
        "a");
}
...
func testTrimMarkers() {
    ...
    testing.assertEqual(
        render(oneSet($src), "main", json.decode('{"xs":["a","b"]}')),
        "<ul>\n  <li>a</li>\n  <li>b</li>\n</ul>");
}
...
func testRootAccessInRange() {
    testing.assertEqual(
        render(
            oneSet('{{ range .xs }}{{ $.site }}:{{ . }} {{ end }}'),
            "main",
            json.decode('{"site":"S","xs":["a","b"]}')),
        "S:a S:b ");
}
...
func testEqNe() {
    testing.assertEqual(
        render(
            oneSet('{{ if eq .role "admin" }}Y{{ else }}N{{ end }}'),
            "main",
            json.decode('{"role":"admin"}')),
        "Y");
    testing.assertEqual(
        render(
            oneSet('{{ if ne .role "admin" }}Y{{ else }}N{{ end }}'),
            "main",
            json.decode('{"role":"user"}')),
        "Y");
}
...
func testCompareNumbers() {
    testing.assertEqual(
        render(
            oneSet('{{ if gt .n 3 }}big{{ else }}small{{ end }}'),
            "main",
            json.decode('{"n":5}')),
        "big");
    testing.assertEqual(
        render(oneSet('{{ if le .n 5 }}ok{{ end }}'), "main", json.decode('{"n":5}')),
        "ok");
}
...
func testAndOrNot() {
    testing.assertEqual(
        render(
            oneSet('{{ if and .a (not .b) }}Y{{ end }}'),
            "main",
            json.decode('{"a":true,"b":false}')),
        "Y");
    testing.assertEqual(
        render(
            oneSet('{{ if or (eq .s "a") (eq .s "b") }}hit{{ end }}'),
            "main",
            json.decode('{"s":"b"}')),
        "hit");
    testing.assertEqual(
        render(
            oneSet('{{ if or (eq .s "a") (eq .s "b") }}hit{{ else }}miss{{ end }}'),
            "main",
            json.decode('{"s":"c"}')),
        "miss");
}
...
func testAssignVariable() {
    testing.assertEqual(
        render(
            oneSet('{{ $t := .title }}{{ range .xs }}{{ $t }}/{{ . }} {{ end }}'),
            "main",
            json.decode('{"title":"T","xs":["a","b"]}')),
        "T/a T/b ");
}
...
func testPipeTruncate() {
    testing.assertEqual(
        render(oneSet('{{ .s | truncate 5 }}'), "main", json.decode('{"s":"hello world"}')),
        "hello...");
    testing.assertEqual(
        render(oneSet('{{ .s | truncate 20 }}'), "main", json.decode('{"s":"short"}')),
        "short");
}
...
func testPrintfPipe() {
    testing.assertEqual(
        render(oneSet('{{ .n | printf "%02d" }}'), "main", json.decode('{"n":7}')),
        "07");
    testing.assertEqual(
        render(oneSet('[{{ .s | printf "%-6s" }}]'), "main", json.decode('{"s":"hi"}')),
        "[hi    ]");
}
...
func testPrintfFunc() {
    ...
    testing.assertEqual(
        ...
        "$3.50");
    testing.assertEqual(
        render(oneSet('[{{ printf "%6s" .s }}]'), "main", json.decode('{"s":"hi"}')),
        "[    hi]");
    testing.assertEqual(
        render(oneSet('{{ printf "%t/%v" .b .n }}'), "main", json.decode('{"b":true,"n":9}')),
        "true/9");
}

func testPrintfLiteralPercent() {
    testing.assertEqual(
        render(oneSet('{{ printf "100%%" }}'), "main", json.decode("null")),
        "100%");
}
...
func testNumberAndBoolLiterals() {
    testing.assertEqual(
        render(oneSet('{{ if eq .flag true }}on{{ end }}'), "main", json.decode('{"flag":true}')),
        "on");
    testing.assertEqual(
        render(oneSet('{{ if lt .n 10 }}under{{ end }}'), "main", json.decode('{"n":7}')),
        "under");
}
...
func throwsMutualInclude() {
    render(
        add(add(newSet(), "a", '{{ template "b" . }}'), "b", '{{ template "a" . }}'),
        "a",
        json.decode("null"));
}
